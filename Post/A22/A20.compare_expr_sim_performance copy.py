...
# Acceleration comparison parameters
DEFAULT_ACC_SIM_FILE = "req_ACC_door_clean.csv"
DEFAULT_ACC_EXPR_COLUMN = "acc@A7_Y.RN_1"
DEFAULT_ACC_SIM_COLUMN = "U3"
...
def plot_acceleration_comparison(
    ...
    expr_column: str,
    sim_column: str,
    start_time_expr: float,
    expr_label: str = "Experiment",
    sim_label: str = "Simulation",
):
    """Plot acceleration comparison between experiment and simulation."""
    textstyle = {"family": "Times New Roman", "size": 32}
    tick_font = "Times New Roman"
    tick_fontsize = 20

    # Plot experiment data - convert to numeric first
    acc_data = pd.to_numeric(df_expr[expr_column], errors='coerce')
    ax.plot(df_expr["t"] - start_time_expr, acc_data*9.8, label=expr_label, color="r", linestyle="--")

    # Plot simulation data
    ax.plot(df_sim["TIME"], df_sim[sim_column]/1000, label=sim_label, color="k", linestyle="-")

    ax.set_xlabel("Time (s)", fontdict=textstyle)
    ax.set_ylabel("Acceleration (m/s²)", fontdict=textstyle)
    ax.legend(fontsize=20, loc="upper left")
    plt.xticks(fontsize=tick_fontsize, fontfamily=tick_font)
    plt.yticks(fontsize=tick_fontsize, fontfamily=tick_font)
    plt.tight_layout()
...
def main():
    ...
    # Apply scaling to experiment acceleration data if specified in config
    if "acc_expr_column" in config and config["acc_expr_column"] in filtered_df_expr.columns:
        acc_scale = scaling_dict.get(config["acc_expr_column"], 1)
        filtered_df_expr[config["acc_expr_column"]] = pd.to_numeric(filtered_df_expr[config["acc_expr_column"]], errors='coerce') / acc_scale
    ...
    # === Acceleration Comparison Figure ===
    print("\n--- Creating Acceleration Comparison ---")

    # Get acceleration parameters from config or use defaults
    acc_sim_file = config.get("acc_sim_file", DEFAULT_ACC_SIM_FILE)
    acc_expr_column = config.get("acc_expr_column", DEFAULT_ACC_EXPR_COLUMN)
    acc_sim_column = config.get("acc_sim_column", DEFAULT_ACC_SIM_COLUMN)

    path_acc_sim = sim_folder / acc_sim_file

    if path_acc_sim.exists():
        print(f"  Loading acceleration sim: {path_acc_sim}")
        df_acc_sim = load_data(path_acc_sim)

        # Filter acceleration simulation data
        filtered_df_acc_sim, _ = filter_data(
            df_acc_sim, SIM_TIME_COL, start_time_sim, end_time_sim, [acc_sim_column], 1
        )

        # Apply scaling to acceleration sim
        filtered_df_acc_sim = apply_scaling(filtered_df_acc_sim, [acc_sim_column], scaling_dict)

        # Apply smoothing to acceleration sim
        filtered_df_acc_sim = apply_smoothing(filtered_df_acc_sim, [acc_sim_column], DEFAULT_SMOOTHING_METHOD, [1])

        # Create acceleration comparison plot
        fig_acc, ax_acc = create_axes()
        ax_acc = plot_acceleration_comparison(
            ax_acc,
            filtered_df_expr,
            filtered_df_acc_sim,
            acc_expr_column,
            acc_sim_column,
            start_time_expr,
            expr_label="Experiment (acc@A7_Y)",
            sim_label="Simulation (U3)",
        )

        # Save acceleration comparison plot
        output_path_acc = output_folder / f"A20.compare_expr_sim_performance_{folder_id}_acc.png"
        fig_acc.savefig(output_path_acc, dpi=300, bbox_inches="tight")
        print(f"✓ Acceleration plot saved to {output_path_acc.resolve()}")
    else:
        print(f"  Warning: Acceleration sim file not found: {path_acc_sim}")
