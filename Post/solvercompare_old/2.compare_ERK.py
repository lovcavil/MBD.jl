...
def test(solver):
    # Load the true data
    df_true = pd.read_csv('./SavedResult/solvercompare/VCABM_high12_run1.csv')
    ...
    # Load the run data
    #solver = "DP5"
    df_run = pd.read_csv(f'./SavedResult/solvercompare/ERK2/{solver}.csv')
    times = df_run['Time']
...
strs=["DP5_run1","Tsit5_run1","TanYam7_run1","DP8_run1",
         "TsitPap8_run1",'VCABM_high12_run1']
for str in strs:
    test(str)