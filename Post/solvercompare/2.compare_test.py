...
def test(path,solver):
    # Load the true data
    df_true = pd.read_csv(f'{path}contact_off_Euler_high.csv')
    ...
    plt.savefig(f'{path}{solver}_tc{len(times)}_rsme{rmse}.png', dpi=500)
    ...
    #plt.legend()
    plt.grid(True, which="both", ls="-")
    plt.savefig(f'{path}{solver}_tc{len(times)}_rsme{rmse}_log.png', dpi=500)
    plt.show()
    
strs=["contact_off_Euler_2024-06-06_13-17-51_run1",
