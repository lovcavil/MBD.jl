...
def export_data(params):
    asc_files, output_folder = params
    TimeS = 'TI FUNCTION_MEA_TIME'
    dfs = {os.path.basename(file): read_structured_file(file) for file in asc_files}
    all_columns = set(col for df in dfs.values() for col in df.columns if col != TimeS)

    for column in all_columns:
        export_df = pd.DataFrame()
        for file_name, df in dfs.items():
            if column in df.columns:
                export_df[file_name] = df[column]
            if TimeS in df.columns:
                export_df['Time'] = df[TimeS]
        export_df.to_csv(f'{output_folder}/{column}_export.csv', index=False)
    
    print(f"Data exported in the folder: {output_folder}")
...
class CSVPlotter:
    def __init__(self, root):
        ...
        self.export_button = Button(self.tab1, text="Export Data", command=self.export_selected_data)
    ...
    def export_selected_data(self):
        ...
        params_list = (selected_files, output_folder)

        export_data(params_list)
