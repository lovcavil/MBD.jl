...
function ExplicitRKFN45(n, tn, Q, Qd, h, hmax, par, SMDT, STSDAT, SJDT, nch)
    ...
    while Err > 1
        ...
        qdd, Lam, ECond = mathfunction.ODEfunct(tnm + c[2] * h, q2p, qd2p, SMDT, STSDAT, SJDT, par)
        k2 = qdd
        q3p = qnm + c[3] * h * qdnm + h^2 * A[3, 1] * k1
        qd3p = qdnm + h * (a1[3, 1] * k1 + a1[3, 2] * k2)
        qdd, Lam, ECond = mathfunction.ODEfunct(tnm + c[3] * h, q3p, qd3p, SMDT, STSDAT, SJDT, par)
        k3 = qdd
        q4p = qnm + c[4] * h * qdnm + h^2 * (A[4, 1] * k1 + A[4, 2] * k2)
        qd4p = qdnm + h * (a1[4, 1] * k1 + a1[4, 2] * k2 + a1[4, 3] * k3)
        qdd, Lam, ECond = mathfunction.ODEfunct(tnm + c[4] * h, q4p, qd4p, SMDT, STSDAT, SJDT, par)
        k4 = qdd
        q5p = qnm + c[5] * h * qdnm + h^2 * (A[5, 1] * k1 + A[5, 2] * k2 + A[5, 3] * k3)
        qd5p = qdnm + h * (a1[5, 1] * k1 + a1[5, 2] * k2 + a1[5, 3] * k3 + a1[5, 4] * k4)
        qdd, Lam, ECond = mathfunction.ODEfunct(tnm + c[5] * h, q5p, qd5p, SMDT, STSDAT, SJDT, par)
        k5 = qdd
        q6p = qnm + c[6] * h * qdnm + h^2 * (A[6, 1] * k1 + A[6, 2] * k2 + A[6, 3] * k3 + A[6, 4] * k4)
        qd6p = qdnm + h * (a1[6, 1] * k1 + a1[6, 2] * k2 + a1[6, 3] * k3 + a1[6, 4] * k4 + a1[6, 5] * k5)
        qdd, Lam, ECond = mathfunction.ODEfunct(tnm + c[6] * h, q6p, qd6p, SMDT, STSDAT, SJDT, par)
        k6 = qdd
        ...
        qd = qdnm + h * k * b1'
        qdd, Lam, ECond = mathfunction.ODEfunct(tnm + h, q, qd, SMDT, STSDAT, SJDT, par)
