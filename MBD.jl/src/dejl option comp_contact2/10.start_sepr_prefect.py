...
from prefect import task, flow, get_run_logger, tags
...
async def set_concurrency_limit(tagname):
    async with get_client() as client:
        # Set a concurrency limit of 28 on the 'test' tag
        limit_id = await client.create_concurrency_limit(
            tag=tagname,
            concurrency_limit=28  # Adjust limit as needed
        )
        print(f"Concurrency limit set for 'test': {limit_id}")

async def check_concurrency_limit(tagname):
    async with get_client() as client:
        concurrency_limit = await client.read_concurrency_limit_by_tag(tagname)
...
async def main_flow(folder,tagname):
    await set_concurrency_limit(tagname)
    logger = get_run_logger()
    ...
    for i, (key, value) in enumerate(json_content.items(), start=1):
        ...
        while True:
            concurrency_available = await check_concurrency_limit(tagname)
...
if __name__ == "__main__":
    ...
    asyncio.run(main_flow(folder,"testc2"))
