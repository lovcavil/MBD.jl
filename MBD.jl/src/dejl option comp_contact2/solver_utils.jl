...
function generate_param_combinations(groups::Vector{<:Dict})
    ...
    solver_group, autodiff_group, linsolve_group, tol_group, timespan_group, initial_state_group,timeout_group,app_group  = groups

    # Use Iterators.product to get the Cartesian product of all groups
    for product_tuple in Iterators.product(solver_group, autodiff_group, linsolve_group, tol_group, timespan_group, initial_state_group,timeout_group,app_group)
        ...
        combined_name = "$(solver_name)_$(autodiff_name)_$(linsolve_name)_$(tol_name)_$(timespan_name)_$(initial_state_name)_$(timeout_name)_$(app_name)"

        # Store the tuple of parameters in the dictionary
        param_combinations[combined_name] = (solver, autodiff, linsolve, abstol, reltol, initial_state, timespan,timeout,app)
    end
...
function convert_to_solver_config(param_combinations)
    ...
    for (combined_name, param_tuple) in param_combinations
        ...
        solver, autodiff, linsolve, abstol, reltol, initial_state, timespan,timeout,app = param_tuple

        # Create the SolverConfig
        solver_configs[combined_name] = SolverConfig(combined_name, solver, autodiff, linsolve, abstol, reltol, 
        initial_state, timespan,timeout,app )
    end
...
function run_solver(config)
    ...
    if function_name=="VCAB3"
        solver=VCAB3()
    elseif function_name=="VCAB4"
        solver=VCAB4()
    elseif function_name=="VCAB5"
        solver=VCAB5()
    elseif function_name=="VCABM3"
        solver=VCABM3()
    elseif function_name=="VCABM4"
        solver=VCABM4()
    elseif function_name=="VCABM5"
        solver=VCABM5()
    elseif function_name=="VCABM"
        solver=VCABM()
    else
    ...
    elapsed_time = @elapsed begin
        integrator,termination_reason = run(tspan, solver;app=app, abstol=abstol, reltol=reltol, timeout=timeout)
    end
...
function run_solvers(solver_configs, folder,csv_filename, n=10)
    ...
    csv_data = [("Method","Solver", "autodiff", "linsolve", "abstol", "reltol", "initial_state", "tspan","app", "ΔE", "Avg Time (s)","Termination_reason", "All Times")]
