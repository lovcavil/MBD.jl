...
function main()
    ...
    tolerance_dict = Dict(
        "tol_1e$i" => [10.0^(-i), 10.0^(-i)] for i in [3,6,9]
    )
    # Timespan Group (Set 3)
    timespan_dict = Dict(
        #"short_timespan" => [0.0, 1.0],
        "short_timespan2" => [0.0, 1.5],
    ...
    param_combinations = generate_param_combinations([solver_dict,autodiff_dict,linsolve_dict, tolerance_dict, timespan_dict,initial_state_dict,timeout_dict,app_dict])
