include("HenonHeiles.jl")  # Include the module file
using .HenonHeiles         # Use the module
...
function main()
    ...
    final_configs = combine_groups([solver_dict,autodiff_dict,linsolve_dict, tolerance_dict, timespan_dict,initial_state_dict])

    # Convert to a list for further use
    solver_configs = collect(values(final_configs))

    # Print the generated configurations for inspection
    # for config in solver_configs
    #     println(config.name, " with solver: ", config.solver, " and tolerances: ", config.abstol, ", ", config.reltol)
    # end

    # Run solvers and store solutions
    sols, energies, times, ΔEs = run_solvers(solver_configs,"solver_results_with_configs3.csv",10)
    println("End")
end
