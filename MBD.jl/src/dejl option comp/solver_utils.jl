...
function run_solvers(solver_configs, folder,csv_filename, n=10)
    ...
    csv_data = [("Solver", "autodiff", "linsolve", "abstol", "reltol", "initial_state", "tspan", "ΔE", "Avg Time (s)","Termination_reason", "All Times")]
    ...
    export_results_to_jld(df, folder,csv_filename)
...
function export_results_to_jld(df, folder,csv_filename)
    # Create the folder path based on the script directory
    folder_path = joinpath(@__DIR__, folder)
