...
function main()
    ...
    param_combinations = generate_param_combinations([solver_dict,autodiff_dict,linsolve_dict, tolerance_dict, timespan_dict,initial_state_dict])
    final_configs = convert_to_solver_config(param_combinations)
    # Convert to a list for further use
    solver_configs = collect(values(final_configs))
    println(param_combinations)
    println(solver_configs)
    # Run solvers and store solutions
    sols, energies, times, ΔEs = run_solvers(solver_configs,"direct","solver_results_with_configs.csv",1)
