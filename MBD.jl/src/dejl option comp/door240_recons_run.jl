...
# function run(tspan, solver; abstol=1e-6, reltol=1e-6)
...
#     integrator = init(prob, solver; callback=CallbackSet(cb1))
...
function run(tspan, solver; abstol=1e-6, reltol=1e-6, timeout=1.0)
    # Constants and initial conditions
    h0, hvar, g = 0.001, 0, 9.81
    contact_json = "contact_g65"
    app = 240
    ...
    integrator = init(prob, solver; callback=CallbackSet(cb1, ), abstol=abstol, reltol=reltol)
