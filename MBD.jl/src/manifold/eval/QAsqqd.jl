...
function QAsqqd(tn, q, qd, SMDT, STSDAT, par)
    ...
    while T <= NTSDA
        ...
        if j >= 1
            ...
            QAsqd = add_constraint!(QAsqd, Q1sq2d, 7 * (i - 1), 7 * (j - 1))
    ...
    while k <= nh    # Contribution from Cylindrical, Revolute, and Translational#Joint Friction
        ...
        if SJDT[1, k] == 4  # Revolute joint
            ...
            Phisr1k = hcat(Phiq1k[:, 1:3]...)
            Phisp1k = hcat(Phiq1k[:, 4:7]...)
            ...
            P4rev511, P4rev512, P4rev522 = bbP4dot2(i, j, vz2pr, s1pr, s2pr, tn, q, Lamk[4], par)
            ...
            if j >= 1
                ...
                Q2krevfrsq2 = [Q2krevfrsq12[:, 8], Q2krevfrsq12[:, 9], Q2krevfrsq12[:, 10],
                    ...
                    Q2krevfrsq12[:, 14]]
                QAsq = add_constraint!(QAsq, Q2krevfrsq1, 7 * (j - 1), 7 * (i - 1))
                QAsq = add_constraint!(QAsq, Q2krevfrsq2, 7 * (j - 1), 7 * (j - 1))
            end
            ...
            Q1krevfrsq1d = [Q1krevfrsq12d[:, 1], Q1krevfrsq12d[:, 2], Q1krevfrsq12d[:, 3],
                Q1krevfrsq12d[:, 4], Q1krevfrsq12d[:, 5], Q1krevfrsq12d[:, 6], Q1krevfrsq12d[:, 7]]
            QAsqd = add_constraint!(QAsqd, Q1krevfrsq1d, 7 * (i - 1), 7 * (i - 1))

            if j >= 1
                Q1krevfrsq2d = [Q1krevfrsq12d[:, 8], Q1krevfrsq12d[:, 9], Q1krevfrsq12d[:, 10],
                    Q1krevfrsq12d[:, 11], Q1krevfrsq12d[:, 12], Q1krevfrsq12d[:, 13],
                    Q1krevfrsq12d[:, 14]]
                QAsqd = add_constraint!(QAsqd, Q1krevfrsq2d, 7 * (i - 1), 7 * (j - 1))
                ...
                Q2krevfrsq2d = [Q2krevfrsq12d[:, 8], Q2krevfrsq12d[:, 9], Q2krevfrsq12d[:, 10],
                    ...
                    Q2krevfrsq12d[:, 14]]
                QAsqd = add_constraint!(QAsqd, Q2krevfrsq1d, 7 * (j - 1), 7 * (i - 1))
                QAsqd = add_constraint!(QAsqd, Q2krevfrsq2d, 7 * (j - 1), 7 * (j - 1))
            end
        end
