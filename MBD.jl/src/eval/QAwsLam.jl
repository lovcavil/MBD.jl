...
function QAwsLamEval(tn, q, qd, Lam, SJDT, STSDAT, par, w, N)
    ...
    while k <= nh
        if SJDT[1, k] == 3
            ...
            QAwsLam = add_constraint!(QAwsLam, Q1kcylfrsLam, 7 * (i - 1), ms - 1)
        ...
        if SJDT[1, k] == 5  # Translational joint
            ...
            csfy2prk, dcsfy2prk = csign(a, par)  # Here 'a' seems out of context, double-check this
            ...
            QAwsLam = add_constraint!(QAwsLam, Q1ktranfrsLam, 7 * (i - 1), ms - 1)
        
            if j >= 1
                Q2ktranfrsLam = [-A1 * vz1pr; -BT2' * A1 * vz1pr] * f12ktranfrsLam
                QAwsLam = add_constraint!(QAwsLam, Q2ktranfrsLam, 7 * (j - 1), ms - 1)
            end
        end

        # Additional joint types can be included here following a similar pattern
        k += 1
    end
