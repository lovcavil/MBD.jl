#####################################################################################
# used for the ideal problem solver compare
# 3 bodies 7 bodies/rollers 1 Handle
# 2 rot | 8 fix| 7 cur-guide
#  *5   |  *6  |     *1   
# contact=0
#####################################################################################
...
function AD340(app,contact_json="contact_off")
    if app >= 340 && app < 349
        script_dir = @__DIR__
        ...
        p_initial = [1.0, 0.0, 0.0, 0.0]  # Common initial 'p' for all bodies
        pd_initial = [0.0, 0.0, 0.0, 0.0]  # Common initial 'pd' (velocity) for all 'p'

        # Initial velocities for 'r' bodies - first body has a unique velocity
        rd_initials = [0.0, 0.0, 0.0]

        rd_initials = [-000.0, 00, 0.0]
...
#####################################################################################        
        ...
        f1=contact_json_data["p_eff"]["f1"]
        AF=contact_json_data["p_eff"]["AF"]
        B5=contact_json_data["p_eff"]["B5"]
        Eeq=contact_json_data["p_eff"]["Eeq"]
        f_start_v=contact_json_data["p_eff"]["f_start_v"]
        f_max=contact_json_data["p_eff"]["f_max"]
        faaa=contact_json_data["p_eff"]["faaa"]
        c_start_delta=contact_json_data["p_eff"]["c_start_delta"]
        c_max_coeff_d=contact_json_data["p_eff"]["c_max_coeff_d"]
        c_damper=contact_json_data["p_eff"]["c_damper"]
        ...
        ld_contact = [contact_mg2, contact_lg, contact_mf, contact_mr]
        damper_mg = Dict(
            "b" => 4,
            "damp" => [0, 0, 0],
        )
        ...
        ld_damper = [damper_door, damper_lg, damper_mf, damper_mr]
